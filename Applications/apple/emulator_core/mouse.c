...
#define COMMAND_SERVE               1
#define COMMAND_CLAMP               2
#define COMMAND_READ                3
#define COMMAND_INIT                4
#define COMMAND_CLEAR               5
#define COMMAND_POS                 6
#define COMMAND_HOME                7
#define COMMAND_SET                 8
#define COMMAND_RESET_FIFO          9
#define COMMAND_GET_RESPONSE        10

//////////////////////////////////////////////////////////////////////////////////////

#define MOUSE_USE_FILE
#define MOUSE_FW_BINARY		"rom/mouse_fw.bin"
#define MOUSE_FW_SOURCE		"fw/mouse_driver/mouse_src.asm"
...
#else
const static uint8_t mouse_fw[4096] =
{
...
static uint8_t fifo_buffer[FIFO_MAX_SIZE];
...
static void write_coord_to_fifo( void )
{
    fifo_buffer[0] = mousex & 0x0FF;
    fifo_buffer[1] = mousex >> 8;
    fifo_buffer[2] = mousey & 0x0FF;
    fifo_buffer[3] = mousey >> 8;
}

static void mouse_io_latch_process( void )
{
    uint32_t t1;
    uint32_t t2;
    ...
    switch( latch_command & 0x0F )
    {
        case COMMAND_NONE:
            if( latch_write_flag == TRUE )
            {
                ...
                if( ++fifo_index >= FIFO_MAX_SIZE ) fifo_index = 0;
            }
            break;
        ...
        case COMMAND_SERVE:
            ...
            fifo_buffer[0] = mousestate | (mouseirq & (IRQMOVE | IRQBUTTON | IRQVBL));
        ...
        case COMMAND_CLAMP:
            t1 = fifo_buffer[2]; //  mouse_mem_dma_read( 0x0578 );
            t1 = ( t1 << 8 ) + fifo_buffer[0]; // mouse_mem_dma_read( 0x0478 );

            t2 = fifo_buffer[3]; // mouse_mem_dma_read( 0x05f8 );
            t2 = ( t2 << 8 ) + fifo_buffer[1]; // mouse_mem_dma_read( 0x04f8 );

            if( ( latch_command & 0x80 ) == 0 )
            {
                // clampxmouse
                mouseleft  = (t1 > t2) ? t2 : t1;
                mouseright = (t1 > t2) ? t1 : t2;
            }
            else
            {
                // clampymouse
                mousetop  = (t1 > t2) ? t2 : t1;
                mousedown = (t1 > t2) ? t1 : t2;
            }
        ...
        case COMMAND_READ:
            ...
            fifo_buffer[4] = mousestate | (mouseirq & (IRQMOVE | IRQBUTTON | IRQVBL));
        ...
        case COMMAND_POS:
            ...
            t2 = fifo_buffer[3]; // mouse_mem_dma_read( 0x05f8 );
            t2 = ( t2 << 8 ) + fifo_buffer[1]; // mouse_mem_dma_read( 0x04f8 );
            mousey = (t2 < mousetop) ? mousetop : (t2 > mousedown) ? mousedown : t2;
        ...
        case COMMAND_SET:
            printf( "mouse set: %02x\n", fifo_buffer[0] );
            mousemode = fifo_buffer[0] & (MODEON | MODEIRQMOVE | MODEIRQBUTTON | MODEIRQVBL);
            fifo_buffer[0] = mousemode;
            keybdrv_mouse_set_state( ( mousemode & MODEON ) ? TRUE : FALSE );
...
static uint8_t mouse_io_controller_2( uint8_t address, uint8_t data )
{
    uint8_t ret8 = 0;
...
static uint8_t mouse_io_controller( uint8_t address, uint8_t data )
{
	...
	switch( address & 0x0F )
	{
		case 0x02:
            ...
            keybdrv_mouse_set_state( ( data & MODEON ) ? TRUE : FALSE );
			break;
...
{
	if( ( board_disc_image_access( MOUSE_FW_BINARY, Disc_Mode_Size, 0, ( uint8_t * )&mouse_fw[0], 256 ) == 0 ) ||
		( force_build == TRUE ) )
	{
		#ifdef ENABLE_EMBEDDED_ASM
...
void mouse_card_insert( uint8_t slot )
{
	...
	#ifdef MOUSE_USE_FILE
    memset( mouse_fw, 0x55, 4096 );
	mouse_card_fw_rebuild( TRUE );
    board_disc_image_access( MOUSE_FW_BINARY, Disc_Mode_Read, 0, ( uint8_t * )&mouse_fw[0], 256 );
	#endif
