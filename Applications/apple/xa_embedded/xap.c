...
static int fgetline(char*,int len, int *rlen, int);
...
int pp_init(void)
{
     ...
     nff=1;
     if(!er) {
          liste=(List*)malloc((long)ANZDEF*sizeof(List));
	  if(!liste) er=E_NOMEM;
     }
     return(er);
}
...
void pp_close(void)
{
	...
	fprintf(stderr, "Blocks not consistent in file %s: start depth=%d, end depth=%d\n",
	 ...
	 file_hdl->close_file( flist[fsp].filep );
...
int icl_open(char *tt)
{
	...
	//setvbuf(fp2,NULL,_IOFBF,BUFSIZE);
     ...
     /* we have to alloc it dynamically to make the name survive another
        pp_open - it's used in the cross-reference list */
     flist[fsp].fname = (char*)malloc(strlen(s+i)+1);
     if(!flist[fsp].fname) {
        return E_FNF;
     }
...
int fgetline(char *t, int len, int *rlen, int fp)
{
     static int c,i;
