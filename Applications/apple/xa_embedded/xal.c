...
int l_addocc(int n, int *v, int *afl) {
     LabOcc *p, *pp;
     ...
     p = ltp->occlist;
     while(p) {
		if (p->line == filep->fline && p->fname == filep->fname) return 0;
		pp = p;
		p = p->next;
     }
     p = (LabOcc*)malloc(sizeof(LabOcc));
     if(!p) {
...
{
	 int er = 0;
     if(crossref) er = l_addocc(n,v,afl);
	 if( er != 0 ) return er;
...
/*printf("ll_def: s=%s\n",s);    */
	...
	afile->la.lti = 0;
	afile->la.ltm = 1000;
	afile->la.lt = (Labtab*)malloc(afile->la.ltm * sizeof(Labtab));
     } 
     if(afile->la.lti>=afile->la.ltm) {
	afile->la.ltm *= 15;
	afile->la.ltm /= 10;
	afile->la.lt = (Labtab*)realloc(afile->la.lt, afile->la.ltm * sizeof(Labtab));
     }
     if(!afile->la.lt) {
		return Asm_ErrorOutOfMemory;
     }
