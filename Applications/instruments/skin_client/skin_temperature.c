...
static void init_gauge_temperature( void )
{
    ...
    temperature_niddle_data.posy = 453;
...
void skin_temperature_set_value( int value )
{
    ...
    // 35 - 95 - 155 = 0 - 1024
    if( value < 35 ) target = 0;
    else if( value > 155 ) target = 1024;
    else target = ( ( value - 35 ) * 8738 ) >> 10;
    
    /*
    if( ( value > 110 ) && ( engine_overheat == FALSE ) )
    {
        engine_overheat = TRUE;
        skin_warning_update( WARNING_SLOT_OVERHEAT, Warning_Temperature_Warning, engine_overheat );
    }
    else if( ( value < 95 ) && ( engine_overheat == TRUE ) )
    {
