	...
	while( 1 )
	{
		if( keyboard_fd_read( fd, &event ) == 0 )
		{
			if( event.type == 2 )
			{
				...
				if( event.code == 0 )
				{
					mouse_data.curr_x += (int)event.value / 2;
				...
				{
					mouse_data.curr_y += (int)event.value / 2;
			...
			else if( event.type == 1 )
			{
				...
				switch( event.code )
				{
					...
					case 272:
                        tthread_mutex_lock( mouse_data.mutex );
                        mouse_data.mouse_button = event.value;
                        mouse_data.is_update = TRUE;
                        tthread_mutex_unlock( mouse_data.mutex );
                        break;
					...
					case 273:
                        tthread_mutex_lock( mouse_data.mutex );
                        mouse_data.mouse_button2 = event.value;
                        mouse_data.is_update = TRUE;
                        tthread_mutex_unlock( mouse_data.mutex );
                        break;
				}
				...
				else
				{
					...
					switch( event.code )
					{
                        ...
                        case 165:
                            mouse_data.start_x  = mouse_data.delta_x;
                            mouse_data.target_x = 128 - (128 * event.value );
                            mouse_data.time_x   = ms_timer_hw_get_ms();
                            break;

                        case 163:
                            mouse_data.start_x  = mouse_data.delta_x;
                            mouse_data.target_x = 128 + (127 * event.value );
                            mouse_data.time_x = ms_timer_hw_get_ms();
                            break;

                        case 164:
                            mouse_data.start_y  = mouse_data.delta_y;
                            mouse_data.target_y = 128 - (128 * event.value );
                            mouse_data.time_y = ms_timer_hw_get_ms();
                            break;

                        case 166:
                            mouse_data.start_y  = mouse_data.delta_y;
                            mouse_data.target_y = 128 + (127 * event.value );
                            mouse_data.time_y = ms_timer_hw_get_ms();
                            break;
					}
		...
		if( keyboard_fd_read( fd, &event ) == 0 )
		{
			if( ( event.type == 1 ) && ( ( event.value == 0 ) || ( event.value == 1 ) ) )
			{
                ...
                switch( event.code )
				{
					...
					case 56:
                        if( event.value ) mouse_data.flags |= OPENAPL_FLAG;
                        else mouse_data.flags &= ~OPENAPL_FLAG;
					    break;

					case 100:
                        if( event.value ) mouse_data.flags |= CLOSEAPL_FLAG;
                        else mouse_data.flags &= ~CLOSEAPL_FLAG;
					    break;
                ...
                if( mask )
				{
					...
					pipe_send_buffer( &kbd_pipe, ( uint8_t * )&kbd_event, sizeof( keyb_event_t ) );
				}
...
void keybdrv_mouse_read_joystick( uint16_t * posx, uint16_t * posy )
{
    ...
    if( mouse_data.time_x != 0 )
    {
        int step_value = ( (int)( ms_timer_hw_get_ms() - mouse_data.time_x ) * 1024 ) / 4500;
        if( mouse_data.start_x > mouse_data.target_x )
        {
            mouse_data.delta_x -= step_value;
            if( mouse_data.delta_x < mouse_data.target_x )
            {
                mouse_data.delta_x = mouse_data.target_x;
                mouse_data.time_x = 0;
            }
        }
        else
    ...
    if( mouse_data.time_y != 0 )
    {
        int step_value = ( (int)( ms_timer_hw_get_ms() - mouse_data.time_y ) * 1024 ) / 4500;
        if( mouse_data.start_y > mouse_data.target_y )
        {
            mouse_data.delta_y -= step_value;
            if( mouse_data.delta_y < mouse_data.target_y )
            {
                mouse_data.delta_y = mouse_data.target_y;
                mouse_data.time_y = 0;
            }
        }
        else
...
bool_t keybdrv_mouse_get_pos( uint16_t * posx, uint16_t * posy, uint16_t * posz )
{
	...
	*posx = (uint16_t)mouse_data.curr_x;
	*posy = (uint16_t)mouse_data.curr_y;
	*posz = mouse_data.mouse_button ? 255 : 0;
...
void keybdrv_init( void )
{
	...
	mouse_data.max_x = mouse_data.max_y = 1024;
	mouse_data.delta_x = mouse_data.delta_y = 128;
	mouse_data.curr_x  = mouse_data.curr_y  = 128;
	mouse_data.is_update = FALSE;
	mouse_data.mouse_button = FALSE;

	mouse_data.open_apple = FALSE;
	mouse_data.close_apple = FALSE;

	mouse_mode = FALSE;

	tthread_create_ex( keyboard_fd_mouse_thread, "/dev/input/event0" );
