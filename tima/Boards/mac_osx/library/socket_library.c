...
uint32_t socket_accept_port( uint32_t socket )
{
    ...
    sa = accept(socket, NULL, NULL);                  // block for connection request
    if ((int)sa < 0)
    {
...
uint32_t socket_connect_url( char * url_addr, uint16_t port )
{
    SOCKET      sa;
    ...
    for(int i = 0; addr_list[i] != NULL; i++)
    {
        ...
        channel.sin_addr.s_addr = inet_addr( ( const char * )inet_ntoa(*addr_list[i]));
    }
...
uint32_t socket_connect_addr( uint8_t * ip_addr, uint16_t port )
{
    ...
    ip_32 = ip_addr[0];
    ip_32 = (ip_32 << 8 ) | ip_addr[1];
    ip_32 = (ip_32 << 8 ) | ip_addr[2];
    ip_32 = (ip_32 << 8 ) | ip_addr[3];
	...
	memset(&channel, 0, sizeof(channel));// zerochannel
	channel.sin_family = AF_INET;
    channel.sin_addr.s_addr = htonl(ip_32);
	channel.sin_port = htons( port );
...
static const device_socket_t system_device_socket =
{
    socket_library_init,
...
#if defined TIMA_OS
SYSTEM_DRIVER_DEFINE( device_socket_t, system_device_socket, socket, "socket", NULL );
#else
device_socket_t * socket_driver = ( device_socket_t * )&system_device_socket;
#endif
