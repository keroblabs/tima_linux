...
#define SDL_PIXEL_CONFIG SDL_PIXELFORMAT_ARGB8888
...
static void sdl_vsync_event( void )
{
    ...
    {
        window_data_t * window_data = ( window_data_t * )sdl_get_window_data_from_index( cnt );
...
static int create_window( window_data_t * window_data )
{
	...
	SDL_PixelFormatEnumToMasks( SDL_PIXEL_CONFIG, &bpp, &r_mask, &g_mask, &b_mask, &a_mask );

    //Create window
    window_data->window = SDL_CreateWindow( "SDL Window", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, _USE_SCREEN_WIDTH, _USE_SCREEN_HEIGHT, 0 );
    ...
    //Get window surface
	window_data->surface = SDL_CreateRGBSurface(0, _USE_SCREEN_WIDTH, _USE_SCREEN_HEIGHT, bpp, r_mask, g_mask, b_mask, a_mask);
	...
	window_data->texture = SDL_CreateTexture( window_data->renderer, SDL_PIXEL_CONFIG, SDL_TEXTUREACCESS_STREAMING, _USE_SCREEN_WIDTH, _USE_SCREEN_HEIGHT);
...
static void sdl_screen_stop_internal( void )
{
...
{
    window_data_t * window_data = sdl_get_window_data();
    if( window_data != NULL ) MFREE( window_data );
    
    window_data = ( window_data_t * )MMALLOC( sizeof( window_data_t ) );
    if( window_data != NULL )
    {
        memset( window_data, 0x00, sizeof( window_data_t ) );
        sdl_set_window_data( window_data );
...
{
    window_data_t * window_data = sdl_get_window_data();
    sdl_instance_send_message( SDL_INTERNAL_DELEGATE, sdl_screen_stop_internal, window_data );
}
